...
| False success from client-supplied chain state | Mint, publication and Owner-control flows resolve transaction/receipt/calldata/event evidence against the configured chain and contract. A shared raw/wait observer preserves included reverts and treats contradictory or unavailable receipt evidence as uncertain. | Verified locally at inclusion level |
