...
The same narrow logger boundary covers Admin authentication, Owner-control and
publication API failures. Authentication events accept only a fixed stage and
failure category. Privileged-operation events accept only a fixed operation and
an allowlisted category; they exclude wallet addresses, transaction hashes,
token/content identifiers, URIs, action values and raw errors. A pending
transaction is normal polling state and is not logged as a failure. These events
are diagnostic signals, not authorization decisions or substitutes for the
append-only chain-evidence audit.
...
structured allowlists, Admin auth, Owner-control and publication failure
classification, response/log redaction and logging-sink isolation. Playwright
verifies the emitted degraded response and safe JSON events from the optimized
server when deployment configuration is intentionally absent. Live provider and
hosted monitoring verification remain pending.
